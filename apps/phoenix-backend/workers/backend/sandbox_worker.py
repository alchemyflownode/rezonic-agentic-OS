...
class SandboxWorker(BaseWorker):
    ...
    async def process(self, task: str, model: str = None, memory_bus=None) -> dict:
        ...
        logger.info(f"⚡[SANDBOX] Waking execution environment...")
        ...
        # 2. Or do we need to pull from the Hive Mind?
            ...
            logger.info("🔍 Searching Hive Mind for recent HandsWorker code...")
        ...
        if not code_to_run:
            return {"content": "❌ **Sandbox Error:** No executable Python code found in prompt or recent memory."}
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                content_msg = f"✅ **Execution Success ({exec_time}s):**\n```text\n{output}\n```"
            ...
            return {"success": False, "content": "⏱️ **Execution Error:** Process timed out after 15 seconds."}
        except Exception as e:
            return {"success": False, "content": f"⚠️ **Sandbox System Error:** {str(e)}"}
