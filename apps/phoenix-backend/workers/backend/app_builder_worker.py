...
class AppBuilderWorker:
    ...
    def _discover_templates(self) -> Dict[str, Path]:
        ...
        logger.info(f"📦 Discovered {len(templates)} app templates")
        return templates
    ...
    async def _build_app(self, args: List[str]) -> dict:
        ...
        if len(args) < 2:
            return {"content": "❌ Usage: /build <template> <app-name>"}
            
        template_name = args[0]
        app_name = args[1]
        
        if template_name not in self.templates:
            return {"content": f"❌ Template '{template_name}' not found. Run /templates to see available."}
        ...
        return {"content": f"""
✅ **SOVEREIGN APP GENERATED: {app_name}**
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
The app has been infused with constitutional awareness! 🦊
"""}
    
    async def _scaffold_project(self, args: List[str]) -> dict:
        """Scaffold a new project with constitutional defaults"""
        if not args:
            return {"content": "❌ Usage: /scaffold <project-name> [--type=react|vue|python]"}
        ...
        return {"content": f"""
🏗️ **SOVEREIGN PROJECT SCAFFOLDED: {project_name}**
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📍 Location: {base_path}
🔧 Type: {project_type}
📋 Structure created with constitutional awareness

The project includes:
  • Constitutional governance hooks
  • RezHiveOS integration points
...
"""}
    ...
    async def _integrate_with_hive(self, args: List[str]) -> dict:
        ...
        if not args:
            return {"content": "❌ Usage: /integrate <app-path>"}
            
        app_path = Path(args[0])
        if not app_path.exists():
            return {"content": f"❌ Path not found: {app_path}"}
            
        # Add Hive integration files
        await self._add_hive_integration(app_path)
        
        return {"content": f"""
🤝 **APP INTEGRATED WITH THE HIVE: {app_path.name}**
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
✅ Added sovereign communication layer
✅ Configured constitutional compliance
✅ Enabled Hive Mind telemetry
✅ Connected to shared memory bus
...
"""}
    ...
    def _list_templates(self) -> str:
        ...
        if not self.templates:
            return "❌ No templates found in G:\\okiru\\app builder"
            
        templates = "\n".join([f"  • {name}" for name in sorted(self.templates.keys())])
...
"""
    ...
    async def _inject_constitutional_components(self, target_path: Path):
        ...
        with open(const_path / "README.md", 'w') as f:
            f.write("""# 🦊 Constitutional Awareness
...
console.log('🦊 Sovereign Node app connected to Hive');
...
""")
    ...
    def _get_help(self) -> str:
        return """🦊 **APP BUILDER - SOVEREIGN GENERATION ENGINE**
...
Your past months of work become sovereign components! 🏗️
"""