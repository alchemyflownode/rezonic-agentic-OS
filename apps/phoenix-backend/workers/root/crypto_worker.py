...
class CryptoWorker:
    ...
    def __init__(self):
        ...
        logger.info("ðŸ“ˆ CryptoWorker mounted. Liquidity nodes connected.")
    ...
    async def execute_paper_trade(self, action: str, symbol: str, amount_usd: float) -> dict:
        ...
        if action == "BUY":
            if self.portfolio["USD"] >= amount_usd:
                ...
                msg = f"âœ… BOUGHT {amount_coin:.6f} {symbol.split('/')[0]} at ${live_price:,.2f}"
            else:
            ...
            if self.portfolio["BTC"] >= amount_coin:
                self.portfolio["USD"] += amount_usd
                self.portfolio["BTC"] -= amount_coin
                msg = f"âœ… SOLD {amount_coin:.6f} {symbol.split('/')[0]} at ${live_price:,.2f}"
            else:
