...
class EvolutionWorker:
    ...
    async def evolve(self, backtest_results: List[Dict[str, Any]]):
        ...
        """
        logger.info(f"ðŸ§¬ Starting evolution cycle #{self.generation}")

        # 1ï¸âƒ£ Select top performers
        ...
        self.generation += 1
        logger.info(f"ðŸ§¬ Evolution cycle #{self.generation} complete. Library size: {len(self.strategy_library)}")
