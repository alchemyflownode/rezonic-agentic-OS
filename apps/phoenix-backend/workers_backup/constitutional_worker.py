...
class ConstitutionalWorker:
    ...
    async def _import_constitutional_docs(self, memory_bus) -> str:
        ...
        return f"âœ… Imported {imported} constitutional documents into Hive Mind"
    
    def _get_violations_report(self) -> str:
        """Generate violations report"""
        if not self.violations:
            return "âœ… No constitutional violations recorded"
            
        report = "ðŸ“‹ **CONSTITUTIONAL VIOLATIONS**\n\n"
        for v in self.violations[-10:]:  # Last 10
            report += f"â€¢ {v['action']}: {v['reason']} (Severity: {v['severity']})\n"
            
        return report
    
    def _get_help(self) -> str:
        return """âš–ï¸ **CONSTITUTIONAL WORKER**
