    ...
    def __init__(self):
        ...
        if HAS_PYNVML:
            try:
                pynvml.nvmlInit()
                self.has_gpu = True
                logger.info(f"ðŸŽ® GPU detected")
            except:
                pass
...
class CodeGenWorker(Worker):
    """Code generation worker with improved intent parsing"""
    def __init__(self):
...
print(f"Fibonacci(10): {result}")'''
        ...
        # Add two numbers
        elif "add" in intent_lower or "sum" in intent_lower:
...
print(f"5 * 3 = {result}")'''
        ...
        # Sort list
        elif "sort" in intent_lower:
            code = '''def sort_list(arr: list) -> list:
...
print(f"Reversed: {result}")'''
        ...
        # Factorial
        elif "factorial" in intent_lower:
...
class PhoenixKernel:
    ...
    async def run(self):
        print("\n" + "="*80)
        print(f"ðŸ”¥ PHOENIX ULTIMATE v{self.version} - 65+ WORKERS EDITION")
        print("="*80)
