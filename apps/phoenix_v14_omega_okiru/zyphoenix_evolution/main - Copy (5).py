...
async def connect(sid, environ):
    await sio.emit('agentLog', {"timestamp": datetime.now().isoformat(), "message": f"Secure Link: {sid[:8]}", "type": "SYSTEM"}, room=sid)
...
async def handle_trade(sid, data):
    ...
    await sio.emit('trade_result', {"status": "AUTHORIZED", "certificate": f"0x{hashlib.sha256(str(time.time()).encode()).hexdigest()[:16]}"}, room=sid)
