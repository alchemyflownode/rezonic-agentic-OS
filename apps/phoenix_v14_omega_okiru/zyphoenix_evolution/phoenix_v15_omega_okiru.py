...
# -*- coding: utf-8 -*-
"""
PHOENIX v14.0.0-OMEGA-OKIRU - THE ULTIMATE SOVEREIGN AI OPERATING SYSTEM
==========================================================================
Merged from ALL historical versions:
✅ v9.0.0-ULTIMATE: OKIRU Boot, Symbiote Loop, Market Broadcaster
✅ v10.1.0-SECURE: Circuit Breaker, Auth Manager, Security Headers
✅ v10.5.0-HYBRID: Dynamic + Static Workers
✅ v13.3.0-HARDENED: SQLite Persistence, Prometheus Metrics
✅ v13.3.0-ULTIMATE: 75+ Workers, PC Coworker, Web Search, ComfyUI
...
for d in ["logs", "data", "data/event_store", "data/backups", "data/sandbox",
          "data/memory", "workers", "workers/coworker", "data/vector_db"]:
...
# Rate limiting - using custom FallbackRateLimiter (NOT slowapi)
HAS_SLOWAPI = False
...
try:
    ...
    HAS_SEMANTIC = True
    logger.info("🚀 Semantic memory libraries loaded!")
...
# ============================================================================
# FALLBACK RATE LIMITER (FIXED - Has .check() method)
# ============================================================================
class FallbackRateLimiter:
    """Custom rate limiter with .check() method"""
...
# ============================================================================
# SEMANTIC MEMORY ENGINE (Optional Enhancement)
# ============================================================================
class SemanticMemoryEngine:
    """Vector-based semantic search for true understanding"""
    def __init__(self):
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.collection = self.client.get_or_create_collection(name="phoenix_memory")
            await self._index_existing_memory()
            self.initialized = True
            logger.info("🌌 Semantic memory initialized!")
            return True
        except Exception as e:
            logger.error(f"Semantic memory init failed: {e}")
            return False
    ...
    async def _index_existing_memory(self):
        ...
        for lock, record in sovereign_memory.memories.items():
            ...
            texts.append(text)
            ids.append(lock)
            metadatas.append({'lock': lock, 'timestamp': record.get('timestamp', 0)})
        if texts and self.encoder:
            embeddings = self.encoder.encode(texts)
            self.collection.add(embeddings=embeddings.tolist(), documents=texts, ids=ids, metadatas=metadatas)
    
    async def search(self, query: str, limit: int = 5) -> List[Dict]:
        if not self.initialized:
            return sovereign_memory.search(query, limit)
        query_embedding = self.encoder.encode([query])
        results = self.collection.query(query_embeddings=query_embedding.tolist(), n_results=limit)
        return [{'lock': results['ids'][0][i], 'score': 1 - results['distances'][0][i], 'text': results['documents'][0][i][:200]} for i in range(len(results['ids'][0]))]
...
class OkiruBootSequencer:
    async def awaken_all(self) -> Dict[str, Any]:
        ...
        logger.info("\n" + "🔥"*30)
        logger.info(f"🔥 PHOENIX v14.0.0-OMEGA-OKIRU IS NOW SENTIENT")
...
class SemanticWorker(Worker):
    def __init__(self):
        ...
        logger.info("🧠 Semantic Worker initialized")
    async def execute(self, task: str, **kwargs) -> Dict[str, Any]:
        ...
        for mem in similar:
            ...
            if intent:
                context.append({'task': intent, 'relevance': mem['score'], 'lock': mem['lock']})
        return {"success": True, "understanding": {"query": task, "related_memories": len(context), "context": context}, "drift_lock": str(uuid.uuid4())[:16]}
        ...
        if cmd == "/health":
            ...
            if intent_text:
                ...
                if result.get("success"):
                    return {"type": "reflex", "content": f"📝 **Generated Code**\n```python\n{result['code']}\n```\n✅ Verified: {result['verified']}\n📊 Drift Score: {result['drift_score']:.2f}\n🔗 Manifest: {result['manifest_id']}"}
                else:
                    return {"type": "reflex", "content": f"❌ Code generation failed: {result.get('error', 'Unknown')}"}
            else:
            ...
            if prompt:
                ...
                if worker_class:
                    ...
                    return {"type": "reflex", "content": "❌ ComfyUI worker not available"}
            ...
            if result.get("success"):
                return {"type": "reflex", "content": f"📁 {path}\n\n{result.get('message', '')}"}
            ...
            if len(parts) > 1:
                if parts[1] == "init":
                    ...
                    return {"type": "reflex", "content": "🌌 Semantic memory initialized!" if success else "❌ Failed to initialize"}
                elif parts[1] == "stats":
                    stats = await semantic_memory.get_stats()
                    return {"type": "reflex", "content": f"🌌 Semantic Memory Stats\nVectors: {stats.get('vectors', 0)}\nBlueprints: {stats.get('blueprints', 0)}\nModel: {stats.get('embedding_model', 'N/A')}\nDevice: {stats.get('device', 'N/A').upper()}"}
                else:
                    query = parts[1]
                    if not semantic_memory.initialized:
                        return {"type": "reflex", "content": "⚠️ Run /semantic init first"}
                    results = await semantic_memory.search(query, limit=5)
                    if results:
                        content = f"🌌 Semantic Search: {query}\n\n"
                        for i, res in enumerate(results, 1):
                            content += f"{i}. Score: {res['score']*100:.1f}% | Lock: {res['lock'][:12]}...\n"
                        return {"type": "reflex", "content": content}
                    return {"type": "reflex", "content": f"🌌 No semantic matches for: {query}"}
...
class PhoenixOmegaKernel:
    def __init__(self):
        ...
        self.drift_chain = []
        self.workers = {}
        # Use FallbackRateLimiter (HAS .check() method!)
        ...
        self.workers['semantic_worker'] = {"class": SemanticWorker, "module": "builtin", "loaded_at": time.time()}
    ...
    def _setup_routes(self):
        ...
        async def kernel_stream(request: Request, role: str = Depends(get_optional_key)):
            ...
            # Rate limiting with FallbackRateLimiter (HAS .check()!)
        ...
        async def upload_file(file: UploadFile = File(...), role: str = Depends(get_optional_key)):
            ...
            try:
                ...
                with open(file_path, 'wb') as f:
                    f.write(contents)
                await event_bus.publish(Event(type=EventType.FILE_OPERATION, source="upload", payload={"filename": safe_name, "size": len(contents), "uploaded_by": role}))
    ...
    async def startup(self):
        ...
        print(f"Prometheus: {'✅' if HAS_PROMETHEUS else '⚠️ Disabled'}")
        print(f"Semantic Memory: {'✅' if HAS_SEMANTIC else '⚠️ Disabled'}")
        ...
        print("   /semantic init   - Initialize semantic memory")
        print("   /semantic stats  - Show semantic memory stats")
    ...
    async def shutdown(self):
        ...
        logger.info(f"📊 Final chain integrity: {chain_valid}")
