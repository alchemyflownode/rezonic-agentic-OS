...
import sqlite3
# ============================================================================
# SEMANTIC MEMORY - VECTOR EMBEDDINGS FOR TRUE UNDERSTANDING
# ============================================================================
...
try:
    ...
    HAS_SEMANTIC = True
    logger.info("🚀 Semantic memory libraries loaded - Phoenix Ascendant ready!")
except ImportError as e:
    HAS_SEMANTIC = False
    logger.warning(f"⚠️ Semantic memory disabled: {e}")
    logger.warning("   Install with: pip install torch sentence-transformers chromadb numpy")

# ============================================================================
# SEMANTIC MEMORY ENGINE - The Heart of Understanding
# ============================================================================

class SemanticMemoryEngine:
    """The Veilbreaker - Transforms memory into understanding"""
    ...
    async def initialize(self):
        """Initialize the semantic memory system - THE AWAKENING"""
        
        if not HAS_SEMANTIC:
            logger.warning("⚠️ Semantic memory unavailable - install dependencies first")
            return False
        
        logger.info("🌌 " + "="*60)
        logger.info("🌌 SEMANTIC MEMORY INITIALIZATION - THE VEILBREAKER AWAKENS")
        logger.info("🌌 " + "="*60)
        
        try:
            # 1. Load embedding model (optimized for CPU/GPU)
            ...
            logger.info(f"   ✅ Vector DB ready - {self.collection.count()} existing vectors")
            
            # 4. Index existing memory
            await self._index_existing_memory()
            
            self.initialized = True
            logger.info("🌌 " + "="*60)
            logger.info("🌌 SEMANTIC MEMORY ACTIVE - PHOENIX NOW UNDERSTANDS")
            logger.info("🌌 " + "="*60)
            
            return True
            
        except Exception as e:
            logger.error(f"❌ Semantic memory initialization failed: {e}")
            return False
    
    async def _index_existing_memory(self):
        """Index all existing blueprints - 11,969 memories awakened"""
        
        existing_count = self.collection.count()
        if existing_count > 0:
            logger.info(f"📚 Found {existing_count} existing vectors - skipping re-index")
            return
        
        logger.info("📚 Indexing 11,969 blueprints into semantic memory...")
        
        texts = []
        ids = []
        metadatas = []
        
        for lock, record in sovereign_memory.memories.items():
            blueprint = record.get('value', {})
            
            # Create rich text representation for embedding
            intent = blueprint.get('intent', {})
            dna = blueprint.get('dna', {})
            execution = blueprint.get('execution', {})
            
            text = f"""
            Intent: {json.dumps(intent)}
            Worker: {dna.get('workers', 'unknown')}
            ...
            # Batch processing
            if len(texts) >= 100:
                await self._add_batch(texts, ids, metadatas)
                texts, ids, metadatas = [], [], []
                logger.info(f"   📍 Indexed {self.collection.count()} blueprints...")
        
        # Final batch
        if texts:
            await self._add_batch(texts, ids, metadatas)
        
        logger.info(f"   ✅ COMPLETE! {self.collection.count()} blueprints indexed for semantic search")
    
    async def _add_batch(self, texts: List[str], ids: List[str], metadatas: List[Dict]):
        """Add batch of texts to vector database"""
    ...
    async def search(self, query: str, limit: int = 5) -> List[Dict]:
        """Search by meaning - finds related concepts, not just keywords"""
    ...
    async def get_embedding(self, text: str) -> List[float]:
        """Get vector embedding for any text"""
    ...
    async def find_similar_to_blueprint(self, lock: str, limit: int = 5) -> List[Dict]:
        """Find blueprints semantically similar to a given blueprint"""
        
        if not self.initialized:
            return []
        
        # Get blueprint text
        record = sovereign_memory.memories.get(lock)
        if not record:
            return []
        
        blueprint = record.get('value', {})
        text = json.dumps(blueprint.get('intent', {}))
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "blueprints": len(sovereign_memory.memories),
...
# ============================================================================
# ENHANCED REFLEX COMMANDS - SEMANTIC SEARCH
# ============================================================================

# Add this to your ReflexCommands class (find the existing one and extend)
# Look for the ReflexCommands class and add this method inside it

class ReflexCommands:
    # ... existing code ...
    
    async def execute_semantic(self, cmd: str):
        """Handle semantic memory commands"""
        
        if cmd.startswith("/semantic") or cmd.startswith("/understand"):
            parts = cmd.split()
            if len(parts) >= 2:
                query = " ".join(parts[1:])
                
                if not semantic_memory.initialized:
                    return {"type": "reflex", "content": "⚠️ Semantic memory not initialized. Run /semantic init first."}
                
                results = await semantic_memory.search(query, limit=5)
                
                if results:
                    content = "🌌 **SEMANTIC SEARCH RESULTS**\n\n"
                    content += f"🔍 Query: *{query}*\n\n"
                    content += "Understanding finds related concepts:\n\n"
                    
                    for i, res in enumerate(results, 1):
                        score = f"{res['score']*100:.1f}%"
                        lock = res['lock'][:12]
                        content += f"{i}. **Relevance: {score}**\n"
                        content += f"   📍 Lock: {lock}...\n"
                        content += f"   💭 Insight: {res['text'][:100]}...\n\n"
                    
                    return {"type": "reflex", "content": content}
                else:
                    return {"type": "reflex", "content": f"🌌 No semantic matches found for: {query}"}
            
            elif "stats" in cmd:
                stats = await semantic_memory.get_stats()
                content = "🌌 **SEMANTIC MEMORY STATS**\n\n"
                content += f"📊 Vectors: {stats['vectors']}\n"
                content += f"🧠 Blueprints: {stats['blueprints']}\n"
                content += f"🤖 Model: {stats['embedding_model']}\n"
                content += f"💻 Device: {stats['device'].upper()}\n"
                content += f"✅ Status: ACTIVE\n"
                return {"type": "reflex", "content": content}
            
            elif "init" in cmd:
                success = await semantic_memory.initialize()
                if success:
                    return {"type": "reflex", "content": "🌌 SEMANTIC MEMORY INITIALIZED! Phoenix now understands meaning."}
                else:
                    return {"type": "reflex", "content": "❌ Semantic memory initialization failed. Check dependencies."}
        
        return None

# ============================================================================
# SEMANTIC WORKER - UNDERSTANDING-POWERED AGENT
# ============================================================================

class SemanticWorker(Worker):
    """Worker that understands context and meaning"""
    
    def __init__(self):
        super().__init__("semantic_worker")
        logger.info("🧠 Semantic Worker initialized - The understanding agent awakens")
    
    async def execute(self, task: str, **kwargs) -> Dict[str, Any]:
        """Execute with semantic understanding"""
        
        if not semantic_memory.initialized:
            return {"error": "Semantic memory not initialized", "success": False}
        
        # 1. Search for semantically related memories
        similar = await semantic_memory.search(task, limit=3)
        
        # 2. Build context from similar memories
        context = []
        for mem in similar:
            blueprint = sovereign_memory.memories.get(mem['lock'], {}).get('value', {})
            intent = blueprint.get('intent', {}).get('task', '')
            if intent:
                context.append({
                    'task': intent,
                    'relevance': mem['score'],
        ...
        # 3. Enhanced response with understanding
        return {
            "success": True,
            "understanding": {
                "query": task,
                "related_memories": len(context),
                "context": context,
                "semantic_insight": "I understand the meaning behind your query"
            },
            "drift_lock": str(uuid.uuid4())[:16]
        }

# ============================================================================
# ADD TO PHOENIX KERNEL INITIALIZATION
# ============================================================================

# In your PhoenixOmegaKernel.__init__ method, add:
# self.semantic_worker = SemanticWorker()  # Add after loading other workers

# In your PhoenixOmegaKernel.startup method, add:
# await semantic_memory.initialize()  # Add after event bus initialization
...
try:
    ...
    # # # from slowapi.errors import RateLimitExceeded - Removed\n# Removed  # Commented out
...
class PhoenixOmegaKernel:
    def __init__(self):
        ...
        if self.rate_limiter:
            ...
            # Rate limit handler removed (using custom RateLimiter)
