...
class ApexWorker(BaseWorker):
    ...
    async def process(self, task: str, **kwargs) -> Dict[str, Any]:
        ...
        if task_lower.startswith("/trade "):
            ...
            self.halted = True
            return {"content": "ðŸ›‘ Trading halted by user command"}
        elif task_lower.startswith("/resume"):
            self.halted = False
            return {"content": "â–¶ï¸ Trading resumed"}
        else:
    ...
    async def _handle_redis_command(self, cmd):
        ...
        if action == 'HALT_ALL':
            self.log("warning", "ðŸ›‘ EMERGENCY HALT RECEIVED")
            self.halted = True
            await self._publish_log('ERROR', 'âš ï¸ BOT HALTED BY KILL SWITCH')
            
        elif action == 'RESUME':
            self.log("info", "â–¶ï¸ Resume command received")
            self.halted = False
            self.error_count = 0
            self.circuit_breaker_active = False
            ...
            try:
                ...
                if spread > 0.001:
                    arbitrage.append({
                        ...
                        "route": "BINANCE â†’ OKX" if binance['last'] < okx['last'] else "OKX â†’ BINANCE",
    ...
    async def _handle_trade_command(self, args: str) -> Dict:
        ...
        parts = args.split()
        if len(parts) < 2:
            return {"content": "âš ï¸ Usage: /trade [buy/sell] [amount]"}
        ...
        if result.get("status") == "success":
            return {"content": f"âœ… Trade executed: {side} {amount} BTC"}
        else:
            return {"content": f"âŒ Trade failed: {result.get('message', 'Unknown error')}"}
    
    async def _get_status(self) -> Dict:
        """Get bot status"""
        return {
            "content": f"""ðŸ¤– **Apex Worker Status**
â€¢ Generation: {self.generation}
â€¢ Status: {'HALTED' if self.halted else 'HUNTING'}
â€¢ Trades: {self.total_trades}
â€¢ Win Rate: {(self.wins/max(self.total_trades,1)*100):.1f}%
â€¢ Circuit Breaker: {'ACTIVE' if self.circuit_breaker_active else 'ARMED'}
â€¢ Balance: ${self.current_balance:,.0f}"""
        }
    ...
    async def _get_risk_metrics(self) -> Dict:
        ...
        risk = await self._calculate_risk()
        return {
            "content": f"""âš–ï¸ **Risk Metrics**
â€¢ Kelly Fraction: {risk['kellyFraction']*100}%
â€¢ Current Drawdown: {risk['currentDrawdown']*100:.1f}%
â€¢ Max Drawdown Limit: {risk['maxDrawdown']*100}%
â€¢ Circuit Breaker: {risk['circuitBreakerStatus']}"""
        }
...
"""
        for ex in market_data:
            response += f"â€¢ {ex['name']}: â‚±{ex['btcPrice']:,.0f} ({ex['latency']}ms) {ex['status']}\n"
        
        if arbitrage:
            response += f"\n**Arbitrage Opportunity:**\n"
            for arb in arbitrage:
                response += f"â€¢ {arb['pair']}: {arb['spread']} ({arb['route']})\n"
        
        response += f"\n**Risk Status:**\n"
        response += f"â€¢ Kelly: {risk['kellyFraction']*100}%\n"
        response += f"â€¢ Drawdown: {risk['currentDrawdown']*100:.1f}%\n"
        response += f"â€¢ Circuit Breaker: {risk['circuitBreakerStatus']}"
