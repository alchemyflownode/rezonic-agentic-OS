...
from core.event_bus import EventBus  # Your kernel's EventBus
from core.circuit_breaker import CircuitBreaker  # Your kernel's CircuitBreaker
...
class TradingEngineWorker:
    ...
    def __init__(self, name: str = "trading_engine"):
        ...
        logger.info(f"🧠 TradingEngineWorker initialized: {name}")
    ...
    def inject_kernel_services(self, event_bus, circuit_breaker, paper_trader):
        ...
        logger.info("✅ TradingEngineWorker connected to kernel services")
    ...
    async def _execute_trade(self, symbol: str, price: float, action: str, 
        ...
        if not self.paper_trader:
            logger.error("❌ Paper trader not available")
            return None
        ...
        try:
            if action == "BUY":
                ...
                if result.get("success"):
                    ...
                    logger.info(f"✅ BUY: {symbol} @ {price} | {reason}")
                ...
                if self.paper_trader.positions.get(symbol, 0) > 0:
                    ...
                    if result.get("success"):
                        ...
                        logger.info(f"✅ SELL: {symbol} @ {price} | {reason}")
                ...
                if self.paper_trader.positions.get(symbol, 0) > 0:
                    ...
                    if result.get("success"):
                        ...
                        logger.info(f"✅ EXIT: {symbol} @ {price} | {reason}")
