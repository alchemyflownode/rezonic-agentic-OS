...
class VisionWorker(BaseWorker):
    ...
    async def analyze_screen(self, prompt: str, monitor: int = 1) -> str:
        logger.info(f"Ã°Å¸â€˜ÂÃ¯Â¸Â Capturing screen for analysis: {prompt}")
    ...
    async def process(self, task: str, model: str = None, memory_bus=None) -> dict:
        ...
        if task_lower in["describe screen", "what do you see", "look"]:
            ...
            content = f"Ã°Å¸â€˜ÂÃ¯Â¸Â **Screen Analysis:**\n\n{analysis_result}"
            ...
            content = f"Ã°Å¸â€Â **Looking for '{element}':**\n\n{analysis_result}"
            ...
            content = f"Ã°Å¸â€œÂ **Text detected:**\n\n{analysis_result}"
            ...
            content = f"Ã°Å¸â€˜ÂÃ¯Â¸Â **Analysis:**\n\n{analysis_result}"
            
        elif task_lower in ["screenshot", "capture"]:
            img = await asyncio.to_thread(self._capture_screen)
            filepath = await asyncio.to_thread(self._save_screenshot, img)
            return {"content": f"Ã°Å¸â€œÂ¸ **Screenshot saved to:**\n`{filepath}`"}
            
        else:
            return {
                "content": "Ã°Å¸â€˜ÂÃ¯Â¸Â **Vision Worker Ready**\nCommands: `describe screen`, `find [element]`, `read text`, `analyze: [q]`, `screenshot`"
            }

        # Ã°Å¸Â§Â  Publish what Vision saw to the Hive Mind
        if self.memory_bus and analysis_result:
            await self.publish(
                content=f"[VISION SAW]: {analysis_result}",
                metadata={"task": task}
            )
            logger.info("Ã°Å¸â€˜ÂÃ¯Â¸Â VisionWorker published insights to Sovereign Context Bus")
