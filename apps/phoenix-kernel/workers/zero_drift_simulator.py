...
class ZeroDriftSimulator:
    ...
    def __init__(self, 
        ...
        logger.info(f"🎯 Zero-Drift Simulator initialized (seed={seed})")
    ...
    async def run_simulation(self, 
        ...
        if sim_key in self._result_cache:
            logger.info(f"📦 Cache hit for {strategy}")
        ...
        logger.info(f"✅ Simulation complete: P&L={pnl_percent:.2f}%, Sharpe={sharpe_ratio:.2f}")
...
async def run_demo():
    ...
    print("\n🔄 Running same simulation again (should be cached)...")
    ...
    print(f"✅ Same result hash: {result2.sim_hash}")
