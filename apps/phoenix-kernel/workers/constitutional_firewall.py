...
class ImmutableAuditLogger:
    ...
    async def start(self):
        ...
        logger.info(f"📝 Immutable audit logger started: {self.audit_path}")
    ...
    async def validate_trade(self, 
        ...
        for v in violations:
            if v.action == ViolationAction.WARN_USER:
                logger.warning(f"⚠️ Constitutional warning: {v.reason}")
            elif v.action == ViolationAction.BLOCK_TRADE:
                logger.error(f"🔴 Trade blocked: {v.reason}")
...
async def demo_firewall():
    ...
    print("\n" + "="*60)
    print("🔐 CONSTITUTIONAL FIREWALL DEMO")
    print("="*60)
    ...
    # Export audit
    print("\n" + "="*60)
    print("📄 AUDIT EXPORT (first 3 entries)")
    print("="*60)
