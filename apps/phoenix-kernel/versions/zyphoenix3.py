...
# -*- coding: utf-8 -*-
"""
PHOENIX ULTIMATE v13.2.1 - FIXED ASYNC INITIALIZATION
...
try:
    ...
    HAS_SOCKETIO = False
    print("âš ï¸ Install: pip install python-socketio for real-time features")
...
try:
    ...
    HAS_PYDANTIC = False
    print("âš ï¸ Install: pip install pydantic pydantic-settings")

try:
    import aiofiles
    import aiofiles.os
except ImportError:
    print("âš ï¸ Install: pip install aiofiles for file operations")
...
try:
    ...
    print("âš ï¸ Install: pip install prometheus-client for metrics")
...
class Settings(BaseSettings):
    ...
    event_persistence_batch: int = 100  # Batch size for event persistence
...
# =======================================================================
# GPU MONITOR (FIXED: No async in __init__)
# =======================================================================
class GPUMonitor:
    def __init__(self):
        self.has_gpu = False
    ...
    def _init_gpu(self):
        try:
            ...
            if device_count > 0:
                ...
                self.has_gpu = True
                self.update_stats()
                logger.info(f"âœ… GPU detected: {self.name}")
        except ImportError:
            logger.info("â„¹ï¸ pynvml not installed - GPU stats disabled")
        except Exception as e:
            logger.info(f"â„¹ï¸ GPU monitoring initialized (background updates will start later): {e}")
    
    async def start_background_updates(self):
        """Start background updates - call this after event loop is running"""
...
# =======================================================================
# EVENT BUS (FIXED: No async in __init__)
# =======================================================================
...
class SovereignEventBus:
    ...
    async def initialize(self):
        """Initialize the event bus - call after event loop is running"""
    ...
    async def _persistence_worker(self):
        """Background worker to persist events to database"""
        batch = []
        while True:
            try:
...
class AuthManager:
    ...
    async def verify_key(self, api_key: Optional[str], client_ip: str = None) -> str:
        ...
        if client_ip:
            ...
            if len(self._rate_limits[client_ip]) > 100:  # 100 requests per minute
    ...
    async def reload_worker(self, worker_name: str) -> bool:
        ...
        async with self._locks[worker_name]:
            if worker_name in self._instances:
                # Re-import and create new instance
                # This would require more sophisticated module reloading
...
# =======================================================================
# =======================================================================
# ENHANCED WORKER LOADER WITH BETTER CLASS DETECTION
# =======================================================================
class WorkerLoader:
    ...
    def is_worker_class(self, obj, class_name: str, module_name: str) -> bool:
        """Determine if a class is a worker using multiple heuristics"""
    ...
    def safe_import_module(self, file_path: Path):
        """Safely import a module with multiple strategies"""
        ...
        try:
            ...
            logger.debug(f"  âš ï¸ Strategy 1 failed for {module_name}: {e}")
        
        # Strategy 2: Import with full path
        try:
            ...
            logger.debug(f"  âš ï¸ Strategy 2 failed for {module_name}: {e}")
        
        # Strategy 3: Import as package
        try:
            import importlib
            module = importlib.import_module(module_name)
            return module
        except Exception as e:
            logger.debug(f"  âš ï¸ Strategy 3 failed for {module_name}: {e}")
    ...
    def load_all(self):
        ...
        # Filter out base/init files but KEEP worker files
        ...
        for py_file in worker_files:
            try:
                ...
                for name, obj in inspect.getmembers(module, inspect.isclass):
                    ...
                    # Skip if it's from another module
                ...
                if worker_classes_found == 0 and classes_found > 0:
                    logger.warning(f"  âš ï¸ No worker classes in {py_file.name} (found {classes_found} other classes)")
                elif classes_found == 0:
                    logger.warning(f"  âš ï¸ No classes at all in {py_file.name}")
        ...
        if self.failed_imports:
            ...
            for f in self.failed_imports[:5]:  # Show first 5
                logger.warning(f"   - {f}")
...
class Constitution:
    ...
    def _record_ruling(self, action: str, ruling: dict):
        ...
        # Publish event asynchronously if event bus is initialized
    ...
    async def _publish_ruling(self, record: dict):
        try:
            ...
            pass  # Event bus might not be initialized yet
...
# =======================================================================
# SOVEREIGN MEMORY (FIXED: No async in __init__)
# =======================================================================
class SovereignMemory:
    ...
    def search(self, query: str, limit: int = 10) -> List[Dict]:
        """Search memory for relevant blueprints"""
        results = []
    ...
    async def start_background_updates(self):
        ...
        if not self._update_task:
            ...
            logger.info("âœ… Memory background updates started")
...
class OllamaClient:
    ...
    async def generate(self, prompt: str, stream: bool = True, system: str = None):
        ...
        # Use circuit breaker for non-streaming calls
        if not stream:
            try:
            ...
            # For streaming, we can't easily use circuit breaker
            try:
...
GPU: {gpu.name if gpu.has_gpu else 'None'} {gpu.temp}Â°C {gpu.utilization}% used
...
class PhoenixKernel:
    def __init__(self):
        ...
        logger.info(f"ðŸš€ Phoenix v{self.version} initialized (sync)")
    ...
    def setup_routes(self):
        """
        Rezonic Agentic Standard - Core API Endpoints
        Defines the primary interface for agents and external systems.
        All endpoints follow the principle of 'Observe, Decide, Act'.
        """
        ...
        async def ollama_status():
            """Status of the connected LLM backend."""
        ...
        async def workers_list(role: str = Depends(auth_manager.require_role("viewer"))):
            ...
            for name, info in self.workers.items():
                workers_info[name] = {
                    ...
                    "metrics": {
                        "calls": 0, # TODO: Fetch from worker_cache
                        "errors": 0,
                        "avg_duration": 0
                    }
                }
        ...
        async def workers_metrics(role: str = Depends(auth_manager.require_role("admin"))):
            """Detailed metrics for all worker agents."""
        ...
        async def constitution_evaluate(request: Request, role: str = Depends(auth_manager.require_role("viewer"))):
            """Evaluate an action against the constitutional rules."""
        ...
        async def router_stats():
            """Statistics on task routing decisions."""
            return self.router.get_stats()

        # ===== MAIN AGENT EXECUTION ENDPOINT (Rezonic Standard Core Loop: Observe -> Decide -> Act) =====
        ...
        async def kernel_stream(
            ...
            """
            Rezonic Agentic Standard - Primary Execution Loop:
            1. Observe: Parse incoming task and context (via authentication/rate limiting).
            2. Decide: Apply constitution, route intent.
            3. Act: Execute via worker or Ollama, then record outcome (SCE Blueprint).
            """
            try:
                data = await request.json()
            except:
            ...
            async def generate():
                ...
                if reflex_result:
                    ...
                    return # Exit early for reflex commands

                # 2. Decide: Constitutional check
                ...
                # 5. Act: Fallback to Ollama if no worker handled it
                if not worker_used:
                ...
                # 6. Observe/Act: Create and store SCE Blueprint (Evidence of Execution)
                blueprint = SCEProtocol.create_blueprint(
                    ...
                    {"response_length": len(full_response) if not worker_used else 0,
                     ...
                     "success": True}, # Simplified success logic for blueprint
        ...
        async def user_feedback(request: Request, role: str = Depends(auth_manager.require_role("viewer"))):
            """Endpoint for users to provide feedback on outcomes."""
            data = await request.json()
        ...
        # ===== OPTIONS HANDLER (CORS) - MOVED HERE AND DEFINED CORRECTLY =====
        ...
        async def options_handler(request: Request, path: str):
            ...
            return JSONResponse(
                content={},
                headers={
                    "Access-Control-Allow-Origin": "*" if settings.environment == "development" else settings.frontend_urls[0],
    ...
    async def startup(self):
        ...
        # Start GPU background updates
        await gpu.start_background_updates()
        
        # Initialize event bus
        await event_bus.initialize()
        
        # Initialize memory background updates
        await self.memory.start_background_updates()
        
        # Initialize Ollama client
        await self.ollama.initialize()
        
        # Publish worker loaded events now that event bus is ready
    ...
    def setup_socketio(self):
        ...
        async def connect(sid, environ, auth):
            ...
            await self.sio.emit('connection_verified', {
                'status': 'ok', 
                'version': self.version
            }, room=sid)
        ...
        async def message(sid, data):
            ...
            # Route and respond
            worker_name = self.router.route(task)
            await self.sio.emit('thinking', {'worker': worker_name}, room=sid)
