...
# In your zyphoenix2.py, replace internal classes with:
...
# Initialize services
event_store = EventStore("data/event_store/events.db")
event_bus = EventBus(event_store)
blueprint_service = BlueprintService(Path("data/memory"))
...
class Settings(BaseSettings):
    ...
    cors_origins: list = ["*"]  # <-- CHANGED: Allow all origins
...
class PhoenixKernel:
    def __init__(self):
        ...
        # CORS - CRITICAL FIX: Allow all origins for development
        self.app.add_middleware(
            CORSMiddleware,
            allow_origins=["*"],  # <-- CHANGED: Force allow all origins
        ...
        if HAS_SOCKETIO:
            self.sio = socketio.AsyncServer(
                cors_allowed_origins="*",  # <-- CHANGED: Allow all for Socket.IO
                async_mode='asgi'
            )
            self.setup_socketio()
    ...
    def setup_routes(self):
        ...
        async def workers_reload(worker_name: str, role: str = None):
        ...
        async def constitution_evaluate(request: Request, role: str = None):
        ...
        async def user_feedback(request: Request, role: str = None):
            data = await request.json()
    ...
    def setup_socketio(self):
        ...
        async def disconnect(sid):
            logger.info(f"ðŸ”´ Disconnected: {sid}")
            if metrics._initialized and metrics.active_connections:
    ...
    async def startup(self):
        ...
        print("\n" + "="*60)
        print(f"ðŸ”¥ PHOENIX v{self.version} - ENHANCED SCE PROTOCOL")
        print("="*60)
