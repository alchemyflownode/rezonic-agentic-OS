...
â•‘  REZPHOENIX KERNEL v15.3.1 - UNIFIED MASTER EDITION [CORRECTED]              â•‘
â•‘  â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•  â•‘
â•‘  The Ultimate Sovereign AI Operating System                                  â•‘
â•‘                                                                              â•‘
â•‘  âœ… ALL SYNTAX ERRORS FIXED                                                  â•‘
â•‘  âœ… Production-Ready | Elite-Tier Architecture                              â•‘
...
class PhoenixFormatter(logging.Formatter):
    def format(self, record):
        ...
        record.emoji = emoji_map.get(record.levelname, "ðŸ“Œ")
...
try:
    ...
    logger.warning("âš ï¸ httpx not installed - Ollama streaming disabled")
...
class SecurityError(Exception):
    """Raised when input validation or security checks fail."""
    pass

def sanitize_input(text: str, max_length: int = 10000, allow_code: bool = False) -> str:
    """Sanitize user input: remove control chars, check path traversal, enforce length."""
...
def secure_filename(filename: str) -> str:
    """Generate safe filename with timestamp prefix and extension validation."""
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
class RateLimiter:
    """Token bucket rate limiter with per-client tracking and automatic cleanup."""
...
class CircuitBreaker:
    """Prevents cascading failures by tracking service health and temporary blocking."""
    def __init__(
        self, name: str,
...
class KillSwitch:
    ...
    async def activate(self, reason: str, triggered_by: str = "system"):
        async with self._lock:
            ...
            self.reason = reason
            logger.critical(f"ðŸ”´ KILL SWITCH ACTIVATED by {triggered_by}: {reason}")
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# SCE PROTOCOL (Drift Locking & Verification) - âœ… FIXED
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
class SCE:
    ...
    def verify(blueprint: dict) -> Dict[str, Any]:
        ...
        return {
            "verified": valid,
            "badge": "ðŸŸ¢ SOVEREIGN" if valid else "ðŸ”´ DRIFTED",
            "drift_lock": stored,
...
class EventBus:
    ...
    async def initialize(self):
        ...
        if table_exists:
            ...
            if column_count == 7:
                logger.info("ðŸ”„ Detected old event schema (7 columns), preserving data")
            elif column_count == 6:
                logger.info("âœ… Event schema is current (6 columns)")
            else:
                logger.warning(f"âš ï¸ Unknown event schema ({column_count} columns)")
        else:
    ...
    async def shutdown(self):
        ...
        if self._store:
            self._store.close()
        logger.info("ðŸ“¦ Event store closed")
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
class Constitution:
    """Hardened pattern-based safety enforcement with policy rulings."""
    def __init__(self):
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# SOVEREIGN MEMORY (Blueprint Storage) - âœ… FIXED
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
class SovereignMemory:
    """Persistent blueprint storage with drift-lock based indexing and search."""
    def __init__(self):
        ...
        for p in DIRS["memory"].glob("*.json"):
            try:
                with open(p, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    if "value" in data:  # âœ… FIX #2: Added 'data' to condition
                        lock = data["value"].get("master_drift_lock")
                        if lock:
                            self.memories[lock] = data
            except Exception as e:
                logger.debug(f"Failed to load {p}: {e}")
        logger.info(f"ðŸ“š Loaded {len(self.memories)} blueprints from disk")
...
class GPUMonitor:
    def __init__(self):
        ...
        try:
            ...
            if self.has_gpu:
                logger.info(f"ðŸ–¥ï¸ GPU detected: {self.gpus[0]['name']} ({self.gpus[0]['total_gb']}GB)")
        except Exception as e:
            self._error = str(e)
    ...
    async def check_connection(self) -> bool:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                self.available = True
                logger.info(f"ðŸ¤– Ollama connected: {len(self.models)} models available")
                return True
        except Exception as e:
...
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# WORKER SYSTEM - âœ… FIXED
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
class Worker(ABC):
    """Abstract base for pluggable task executors with health metrics."""
    ...
    async def health_check(self) -> Dict:
        return {
            ...
            "errors": self.error_count,  # âœ… FIX #3: Added '_count' suffix
            ...
            try:
                ...
                for name, obj in inspect.getmembers(mod, inspect.isclass):
                    ...
                    try:
                        ...
                        logger.info(f"  âœ… Worker loaded: {worker_name}")
                    except Exception as e:
                        logger.debug(f"  âš ï¸ Cannot instantiate {name}: {e}")
            except Exception as e:
                logger.debug(f"  âš ï¸ Skip {py_file.name}: {e}")
        return workers
...
async def api_v1_portfolio():
    from workers.paper_trader_worker import paper_trader
    return await paper_trader.get_portfolio()

@app.post("/api/v1/trade")
async def api_v1_trade(action: str, symbol: str, amount: float, price: float = None):
    from workers.paper_trader_worker import paper_trader
    return await paper_trader.execute_trade(action, symbol, amount, price)
...
class Reflex:
    ...
    async def try_execute(self, cmd: str) -> Optional[Dict]:
        ...
        if lower == "/health":
            stats = gpu.stats()
            return self._response(
                f"ðŸ”¥ **Phoenix v{cfg.VERSION}**\n"
                f"Workers: {len(self.kernel.workers)}\n"
                f"Blueprints: {len(memory.memories)}\n"
                f"Ollama: {'ðŸŸ¢' if ollama.available else 'ðŸ”´'}\n"
        ...
        if lower.startswith("/search "):
            ...
            if results:
                lines = [
                    f"  {i+1}. {r['lock'][:12]}â€¦ (score: {r['score']})"
        ...
        if lower.startswith("/vscode "):
            ...
            if ":" in mode_and_prompt:
                ...
                if worker:
                    ...
                    return self._response(f"ðŸ–¥ï¸ VS Code + Copilot\n{json.dumps(result, indent=2)}")
            return self._response(
...
class PhoenixKernel:
    ...
    def _setup_routes(self):
        ...
        async def memory_stats():
            ...
            try:
                ...
                recent = []
                for lock, data in sorted(memory.memories.items(), key=lambda x: x[1].get("timestamp", 0), reverse=True)[:10]:
                    recent.append({
                        "lock": lock[:16] + "...",
    ...
    async def startup(self):
        ...
        if HAS_PROMETHEUS:
            try:
                ...
                logger.info(f"ðŸ“Š Prometheus metrics on http://{cfg.HOST}:{cfg.METRICS_PORT}")
            except Exception as e:
    ...
    async def shutdown(self):
        ...
        logger.info(f"â›“ï¸ Final chain integrity: {'âœ… VALID' if valid else 'âŒ BROKEN'}")
    ...
    def _print_banner(self):
        ...
        print(f"  Ollama:       {'ðŸŸ¢ ' + ollama.model if ollama.available else 'ðŸ”´ offline'}")
        if gpu.has_gpu:
            s = gpu.stats()
...
if __name__ == "__main__":
    ...
    try:
        ...
        sys.exit(1)
# ==========================================
# API v1 Routes - Added for Constitutional Trading
# ==========================================
...
async def startup_consciousness():
    """Start trading consciousness on kernel startup"""
    from workers.trading_consciousness import trading_consciousness
    await trading_consciousness.start_consciousness()
    logger.info("?? Trading Consciousness activated on startup")


# ==========================================
# TRADING CONSCIOUSNESS ROUTES - DIRECT ADD
# ==========================================
...
async def consciousness_control(action: str):
    ...
    if action == "start":
        ...
        return await trading_consciousness.stop_consciousness()
    else:
...
async def consciousness_insights():
    ...
    return {
        ...
        "recommended_action": "Analyzing market conditions...",
