...
# ADD THIS TO YOUR EXISTING SYSTEM - DON'T REWRITE v13

class IntentParser:
    """Hybrid rule + LLM intent parser"""
    
    def __init__(self):
        self.rules = {
            r'(?:search|find|lookup|what is)': 'SEARCH',
            r'(?:code|generate|function|def)': 'CODE',
            r'(?:file|list|dir|read|write)': 'FILE',
            r'(?:system|cpu|memory|ram)': 'SYSTEM',
            r'(?:gpu|optimize|model)': 'GPU',
        }
    
    def parse(self, intent: str) -> List[Dict]:
        """Parse intent into tasks"""
        tasks = []
        
        # Rule-based first (fast path)
        for pattern, task_type in self.rules.items():
        ...
        # If complex, use LLM (slow path)
        if not tasks or self._is_complex(intent):
            tasks = self._llm_parse(intent)
        
        return tasks
    
    def _is_complex(self, intent: str) -> bool:
        """Detect if intent needs LLM"""
        words = intent.split()
        return len(words) > 10 or 'and' in intent or 'then' in intent
...
# Add these dataclasses right after your Config class and before SCERefiner
...
class Task:
    """Atomic unit of work"""
    id: str = ""
    type: str = ""  # SEARCH, CODE, FILE, SYSTEM, MEMORY, GPU
    description: str = ""
...
class DriftDetector:
    """Detects drift from expected patterns"""
    
    def __init__(self, threshold: float = 0.3):
        self.threshold = threshold
    
    async def check(self, plan: OrchestrationPlan) -> float:
        """Check drift score (0-1)"""
        # Simple drift detection based on task count and complexity
        drift = min(1.0, len(plan.tasks) / 20.0)
        return drift
    
    async def check_single(self, result: Dict) -> float:
        """Check drift for single result"""
        if result.get("error"):
            return 1.0
...
# ============================================================================
# FIX THE CONSTITUTION CLASS (add validate method)
# ============================================================================
...
# ============================================================================
# NOW YOUR SCERefiner WILL WORK
# ============================================================================

class SCERefiner:
    """Active SCE enforcement - NOT just formatting"""
    
    def __init__(self):
        self.constitution = Constitution()
        self.drift_detector = DriftDetector()
    
    async def refine(self, plan: OrchestrationPlan) -> RefinedPlan:
        """Three-stage refinement"""
        
        # 1. Validate against constitution
        validation = await self.constitution.validate(plan)
        if not validation['approved']:
            raise ConstitutionalError(validation['reason'])
        
        # 2. Detect drift
        drift_score = await self.drift_detector.check(plan)
        
        # 3. Create executable blueprint using SCEProtocol
        blueprint = SCEProtocol.create_blueprint(
            intent=plan.to_dict(),
            dna={'workers': len(plan.tasks)},
            execution={'mode': 'orchestrated'}
        )
        
        # Add drift correction if needed
        drift_corrected = drift_score > config.DRIFT_THRESHOLD if hasattr(config, 'DRIFT_THRESHOLD') else False
        ...
        return RefinedPlan(
            ...
            sce_lock=blueprint.get('master_drift_lock', ''),
            refined_tasks=plan.tasks
        )
    
    async def validate_result(self, result: Dict) -> bool:
        """Validate a result against constitution"""
        if result.get("error"):
            return False
        return True
    
    async def correct_drift(self, result: Dict) -> Dict:
        """Attempt to correct drift in result"""
        result["drift_corrected"] = True
        result["correction_notes"] = "Automatic drift correction applied"
        return result
...
# hive_bridge.py
# CONNECTS ORCHESTRATOR TO YOUR EXISTING WORKERS

class HiveBridge:
    """Connects orchestration to your v13 workers"""
    
    def __init__(self, kernel: PhoenixKernel):
        self.kernel = kernel  # Your existing kernel
        self.worker_map = self._build_worker_map()
    
    async def execute_plan(self, plan: OrchestrationPlan) -> Dict:
        """Execute orchestration plan using your workers"""
        
        results = {}
        
        for task in plan.tasks:
            # Map task to your existing worker
            worker_name = self._map_to_worker(task['type'])
            
            if worker_name in self.kernel.workers:
                # Call your existing worker
                worker = self.kernel.workers[worker_name]['class']()
                result = await worker.execute(task['description'])
                results[task['id']] = result
        
        return results
    
    def _map_to_worker(self, task_type: str) -> str:
        """Map orchestration task to your worker names"""
        mapping = {
            'SEARCH': 'duckduckgo',
            'CODE': 'code_gen',
            'FILE': 'enhanced_filesystem',
...
# streaming_bridge.py
# CONNECTS YOUR BACKEND TO REACT UI

class StreamingBridge:
    """Real-time execution streaming to UI"""
...
# Add to your PhoenixKernel _setup_routes()

@self.app.websocket("/hive/stream")
async def hive_stream(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            async for event in self.nervous_system.stream_execute(intent):
...
class ClipboardWorker(Worker):
    ...
    async def execute(self, task: str, **kwargs) -> Dict[str, Any]:
        if not self.has_pyperclip:
            return {"error": "pyperclip not installed", "success": False}
...
# These are the NEW layers you added - they need to be instantiated

self.intent_parser = IntentParser()           # Your new orchestrator
self.sce_refiner = SCERefiner()               # Your new SCE wrapper  
self.hive_bridge = HiveBridge(self)           # Connects to your workers
self.streaming_bridge = StreamingBridge()     # Connects to UI
...
async def hive_stream(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # Parse intent
            tasks = self.intent_parser.parse(intent)
            ...
            # Refine with SCE
            refined = await self.sce_refiner.refine(plan)
            
            # Execute via hive bridge
            results = await self.hive_bridge.execute_plan(refined)
            
            # Stream results
            await websocket.send_json({
                'type': 'complete',
                'results': results,
                'sce_lock': refined.blueprint.get('master_drift_lock')
            })
