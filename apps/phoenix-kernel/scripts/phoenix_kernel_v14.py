...
# ============================================================================
# BASE WORKER CLASS (MUST COME BEFORE ALL WORKERS)
# ============================================================================
...
# ============================================================================
# REZCODE COMPILER - THE INTENT COMPILER
# ============================================================================

@dataclass
class CodeIntent:
    """Raw code generation intent"""
    description: str
    language: str = "python"
...
class RezCodeCompiler:
    """
    The Sovereign Code Compiler
    Turns intent → deterministic manifest that workers execute
    """
    ...
    async def compile(self, intent: CodeIntent) -> CodeManifest:
        """Compile intent into deterministic manifest"""
        
        # Parse intent type
        intent_type = self._parse_intent_type(intent.description)
        
        # Infer or use provided signature
...
# ============================================================================
# REZ SWARM WORKER - GPU OPTIMIZATION
# ============================================================================

class RezSwarmWorker(Worker):
    """
    Rez Swarm GPU Optimization Worker
    Applies sparse attention and TurboSparse optimization for 40-60% VRAM reduction
    """
    ...
    async def _safe_delete(self, path: str) -> Dict[str, Any]:
        try:
            ...
            if p.is_file():
                ...
                return {"success": True, "message": f"🗑️ Moved to trash: {p.name}"}
            elif p.is_dir():
                shutil.rmtree(str(p))
...
class SystemMonitorWorker(Worker):
    """System monitoring and PC control - REAL DATA ONLY"""
    ...
    async def _get_real_system_info(self) -> Dict[str, Any]:
        """Get REAL system information - NO SIMULATION"""
        try:
            import platform
            import psutil
            from datetime import datetime
            ...
            # Get REAL CPU model from Windows
            cpu_model = "Unknown"
            try:
                import subprocess
    ...
    async def _copy(self, text: str) -> Dict[str, Any]:
        try:
            ...
            if len(self.history) > 50:
                self.history.pop(0)
            return {"success": True, "message": f"📋 Copied {len(text)} chars", "preview": text[:100]}
        except Exception as e:
            return {"error": str(e), "success": False}
...
# ============================================================================
# CODE GEN WORKER (Executes Manifests)
# ============================================================================

class CodeGenWorker(Worker):
    """Generates code from RezCode manifests"""
    ...
    async def execute(self, task: str, **kwargs) -> Dict[str, Any]:
        """Execute code generation from intent or manifest"""
        
        # If manifest provided directly
        if "manifest" in kwargs:
            manifest = kwargs["manifest"]
            return await self._execute_manifest(manifest)
        
        # Otherwise compile intent first
    ...
    async def _execute_manifest(self, manifest: CodeManifest) -> Dict[str, Any]:
        ...
        # Generate code (simplified - would call actual model)
    ...
    def _generate_code(self, spec: Dict[str, Any]) -> str:
        """Generate code from spec (simplified)"""
        if "add" in spec.get("signature", ""):
            return '''def add(a: int, b: int) -> int:
    """Add two numbers."""
    return a + b'''
...
class DuckDuckGoWorker(Worker):
    """Web search worker using DuckDuckGo"""
...
class OllamaClient:
    ...
    async def generate(self, prompt: str, system: str = None, stream: bool = True, use_turbo: bool = True):
        """Generate with optional TurboSparse optimization"""
...
class ReflexCommands:
    ...
    async def execute(self, cmd: str):
        ...
        if cmd == "/health":
            ...
            return {"type": "reflex", "content": f"📚 Memory: {len(sovereign_memory.memories)} blueprints stored"}
        
        # Rez Swarm command
        elif cmd.startswith("/rezswarm"):
            parts = cmd.split()
            if len(parts) > 1 and parts[1] == "status":
                worker = RezSwarmWorker()
                result = await worker.execute("status")
                if result.get("success"):
                    return {"type": "reflex", "content": f"🚀 **Rez Swarm Status**\n\nActive: {result['active']}\nSparsity: {result['sparsity']}\nOptimizations: {result['optimizations']}\nVRAM Saved: {result['vram_saved_gb']:.1f} GB"}
            else:
                return {"type": "reflex", "content": "📊 Rez Swarm: /rezswarm status"}
        ...
        # File commands
            ...
            parts = cmd.replace("/write", "").strip().split(" ", 1)
            if len(parts) == 2:
                filename, content = parts
                worker = EnhancedFileSystemWorker()
                result = await worker.execute("write file", filename=filename, content=content)
                if result.get("success"):
                    return {"type": "reflex", "content": f"✅ Saved: {filename}"}
        ...
        # System commands
            ...
            worker = SystemMonitorWorker()
            result = await worker.execute("open app", app=app)
            if result.get("success"):
                return {"type": "reflex", "content": f"✅ Opened: {app}"}
        
        elif cmd.startswith("/close"):
            app = cmd.replace("/close", "").strip()
            worker = SystemMonitorWorker()
            result = await worker.execute("close app", app=app)
            if result.get("success"):
                return {"type": "reflex", "content": f"✅ Closed: {app}"}
        ...
        # Clipboard commands
            ...
            if text:
                ...
                result = await worker.execute("copy text", text=text)
                if result.get("success"):
                    return {"type": "reflex", "content": f"📋 Copied: {text[:100]}"}
        
        elif cmd == "/paste":
            worker = ClipboardWorker()
            result = await worker.execute("paste text")
            if result.get("success"):
                return {"type": "reflex", "content": f"📋 Clipboard: {result.get('text', '')[:500]}"}
        ...
        # Browser commands
            ...
            url = cmd.replace("/openurl", "").strip()
            if url:
                worker = BrowserWorker()
                result = await worker.execute("open url", url=url)
                if result.get("success"):
                    return {"type": "reflex", "content": f"🌐 Opened: {url}"}
        
        return None
    
    def _format_dir_listing(self, result: Dict) -> str:
        lines = [f"📁 {result['path']}", "="*50]
...
🕐 Boot Time: {result.get('boot_time', 'Unknown')}
⏱️ Uptime: {result.get('uptime_seconds', 0) / 3600:.1f} hours"""
    
    def _format_cpu_info(self, result: Dict) -> str:
        """Format CPU info"""
        lines = [f"💻 CPU INFORMATION", "="*50]
        ...
        if per_core:
            ...
            for i, core in enumerate(per_core):
                bar = "█" * int(core / 10) + "░" * (10 - int(core / 10))
    ...
    def _format_memory_info(self, result: Dict) -> str:
        ...
        ram_bar = "█" * int(ram.get('percent', 0) / 10) + "░" * (10 - int(ram.get('percent', 0) / 10))
        swap_bar = "█" * int(swap.get('percent', 0) / 10) + "░" * (10 - int(swap.get('percent', 0) / 10))
...
Swap:
   Total:   {swap.get('total_gb', 0):>6.1f} GB
   Used:    {swap.get('used_gb', 0):>6.1f} GB
   Free:    {swap.get('free_gb', 0):>6.1f} GB
   Usage:   [{swap_bar}] {swap.get('percent', 0)}%"""
    ...
    def _format_disk_info(self, result: Dict) -> str:
        ...
        for d in disks:
            ...
            lines.append(f"\n📀 {d.get('device', 'Unknown')} ({d.get('mountpoint', '/')})")
    ...
    def _format_processes(self, result: Dict) -> str:
        ...
        for p in processes[:15]:
            ...
            pid = p.get('pid', '?')
            bar = "█" * int(cpu / 5) + "░" * (20 - int(cpu / 5)) if cpu < 100 else "█" * 20
...
class PhoenixKernel:
    def __init__(self):
        ...
        # Add RezCode Compiler (core module)
    ...
    def _setup_routes(self):
        ...
        async def health():
            vram = gpu_monitor.get_vram_summary()
            # Get Rez Swarm status if available
            rez_swarm_status = {}
...
Be concise, wise, and helpful."""
    ...
    async def run(self):
        ...
        print("\n" + "="*80)
        print(f"🔥 PHOENIX ULTIMATE v{self.version} - THE ULTIMATE SWARM + PC COWORKER + REZCODE + REZ SWARM")
        print("="*80)
        ...
        print(f"Auto-Search: {'✅' if config.ENABLE_AUTO_SEARCH else '❌'}")
        print(f"PC Coworker: ✅ File Manager, System Monitor, Clipboard, Browser")
        print(f"RezCode: ✅ Intent Compiler + Code Generator")
        print(f"Rez Swarm: {'✅ Active' if config.REZ_SWARM_ENABLED else '❌ Disabled'} (sparsity: {config.REZ_SWARM_SPARSITY})")
        print("="*80)
