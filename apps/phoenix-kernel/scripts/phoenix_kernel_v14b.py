...
class SystemMonitorWorker(Worker):
    ...
    async def _get_processes(self) -> Dict[str, Any]:
        try:
            ...
            return {"success": True, "processes": procs[:20], "total": len(procs)}
        except Exception as e:
            return {"error": str(e), "success": False}
...
class ReflexCommands:
    ...
    async def execute(self, cmd: str):
        ...
        # System commands
            ...
            result = await worker.execute("get system info")
            if result.get("success"):
            ...
            worker = SystemMonitorWorker()
            result = await worker.execute("get cpu info")
            if result.get("success"):
                return {"type": "reflex", "content": f"💻 CPU: {result.get('percent')}%\nCores: {result.get('cores')}\nPer CPU: {result.get('per_cpu', [])}"}
        
        elif cmd == "/memory" or cmd == "/ram":
            worker = SystemMonitorWorker()
            result = await worker.execute("get memory info")
            if result.get("success"):
                return {"type": "reflex", "content": f"💾 RAM: {result.get('used_gb')}GB / {result.get('total_gb')}GB ({result.get('percent')}%)"}
        
        elif cmd == "/disk":
            worker = SystemMonitorWorker()
            result = await worker.execute("get disk info")
            if result.get("success"):
                disks = result.get("disks", [])
                if disks:
                    d = disks[0]
                    return {"type": "reflex", "content": f"💿 Disk: {d.get('free_gb')}GB free / {d.get('total_gb')}GB total ({d.get('percent')}%)"}
        
        elif cmd == "/processes":
            worker = SystemMonitorWorker()
            result = await worker.execute("get processes")
            if result.get("success"):
                procs = result.get("processes", [])[:10]
                lines = ["📊 Top Processes:"]
                for p in procs:
                    lines.append(f"  • {p.get('name')} (PID: {p.get('pid')}) - CPU: {p.get('cpu_percent', 0)}%")
