...
// 🔥 FIXED PORT TO MATCH YOUR BACKEND
    ...
    >
      ...
      <div className="max-w-6xl w-full flex flex-col gap-12 relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-[10px] text-[#00E5FF] tracking-widest uppercase">Boot Sequence Initiated • ULTIMATE EDITION v10.6.2</p>
        </div>
...
export default function SovereignDashboard() {
  ...
  // Boot & System State
  const [bootPhase, setBootPhase] = useState(0); // 0-5
  ...
  useEffect(() => {
    ...
    const fetchSystemState = async () => {
      try {
        ...
        if (healthRes.ok) {
          ...
          const rulingsRes = await fetch(`${API_BASE}/constitution/history?limit=5`, { headers });
          if (rulingsRes.ok) {
            const data = await rulingsRes.json();
            setRulings(data.rulings ||[]);
          }
          
          const eventRes = await fetch(`${API_BASE}/events/stats`, { headers });
          if (eventRes.ok) {
    ...
    if (!isBooting) {
      fetchSystemState();
      const pollInterval = setInterval(fetchSystemState, 5000);
  ...
  // Update Sparkline
  useEffect(() => {
    const currentRate = eventStats.total_events > 0 ? Math.floor(Math.random() * 10) + 1 : 0; // Simulated active rate if backend static
  ...
  // 🔥 FIXED handleTransmit with thinking animation
  const handleTransmit = async () => {
    ...
    // Trigger Visual Overlays based on intent
    ...
    try {
      ...
      const evalRes = await fetch(`${API_BASE}/constitution/evaluate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Hive-API-Key': API_KEY },
        body: JSON.stringify({ action_type: 'chat_query', agent_id: 'sovereign', payload: { query: userText } })
      });
      const ruling = await evalRes.json();

      if (ruling.decision !== 'AUTHORIZED') {
        setMessages(prev =>[...prev, { 
          id: Date.now().toString(), 
          role: 'system', 
          content: `🚨 BLOCKED: ${ruling.reasoning}`, 
          timestamp: new Date().toLocaleTimeString('en-US', { hour12: false }) 
        }]);
        setGovScore(Math.max(0, govScore - 25)); // Visual penalty
        setIsStreaming(false);
        return;
      }
      
      setGovScore(Math.min(100, govScore + 5)); // Visual reward
      ...
      else if (userText.startsWith('/memory') || userText.includes('remember') || userText.includes('recall')) worker = 'cortex';
      ...
      const streamRes = await fetch(`${API_BASE}/kernel/stream`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Hive-API-Key': API_KEY },
        body: JSON.stringify({ 
          task: userText, 
          ...
          if (event.startsWith('data: ')) {
            try {
              ...
              if (data.type === 'thinking') {
                // Update thinking state with worker info
                setMessages(prev => prev.map(msg => 
                  msg.id === assistantId ? { 
                    ...msg, 
                    content: '⏳ Processing...',
                    narrative: [`Using ${data.worker || worker} worker`]
                  } : msg
                ));
              }
              else if (data.type === 'result') {
                // Show final result with drift lock
                ...
                setMessages(prev => prev.map(msg => 
                  msg.id === assistantId ? { 
                    ...
                    narrative: [] // Clear thinking state
                  } : msg
                ));
              }
              else if (data.error) {
                fullContent += `\n🚨 KERNEL ERROR: ${data.error}`;
                setMessages(prev => prev.map(msg => 
                  msg.id === assistantId ? { 
                    ...msg, 
                    content: fullContent,
  ...
  // Mock data for heatmap if backend is sparse
      ...
      <main className="relative z-10 flex-1 flex gap-4 min-h-0">
        ...
        <motion.div {...fadeUp} transition={{ ...springSoft, delay: 0.2 }} className="flex-1 bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl flex flex-col relative overflow-hidden shadow-lg min-w-0 min-h-0">
          ...
          <div className="h-12 border-b border-white/5 flex items-center justify-between px-4 shrink-0 bg-white/[0.01]">
            ...
            <div className="flex items-center gap-4"><span className="text-[9px] font-mono text-zinc-500 tracking-wider">ENGINE: <span className="text-[#A78BFA]">LLAMA3.2:LATEST</span></span></div>
          </div>
