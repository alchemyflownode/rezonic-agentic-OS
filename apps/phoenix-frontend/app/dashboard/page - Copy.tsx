...
async function fetchWithKey(endpoint: string, options: RequestInit = {}) {
  const headers = {
...
interface Message {
  id: string;
  role: 'user' | 'assistant' | 'system';  // fixed: 'user' not 'null'
  content: string;
  narrative?: string[];
  driftLock?: string;
  timestamp: string;
        ...
        <div className="w-full max-w-3xl flex justify-between items-center relative z-10">
          ...
          <div className="flex flex-col gap-4">
            <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.6 }} className="px-4 py-6 bg-gradient-to-br from-[#00E5FF]/10 to-[#00E5FF]/5 border border-[#00E5FF]/30 rounded-lg flex flex-col items-center gap-3 w-48 shadow-[0_0_20px_rgba(0,229,255,0.1)] relative">
              ...
              {/* Coworker Workers Section */}
        <div className="bg-[#24283b]/50 rounded-xl border border-[#292e42] p-6 mt-6">
          <h2 className="text-xl font-semibold text-[#bb9af7] mb-4 flex items-center gap-2">
            <Users className="w-5 h-5" />
            Coworker AI Agents
          </h2>
          <p className="text-[#565f89] text-sm mb-4">
            {coworkerCount} specialized AI agents ready to assist with tasks
          </p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {coworkerWorkers.slice(0, 8).map((worker, i) => (
              <div key={i} className="bg-[#1a1b26] rounded-lg p-2 text-center border border-[#292e42]">
                <div className="text-xs text-[#c0caf5] truncate">{worker}</div>
              </div>
            ))}
            {coworkerWorkers.length > 8 && (
              <div className="bg-[#1a1b26] rounded-lg p-2 text-center border border-[#292e42]">
                <div className="text-xs text-[#565f89]">+{coworkerWorkers.length - 8} more</div>
              </div>
            )}
          </div>
          <div className="mt-4">
            <a href="/workers" className="text-sm text-[#7dcfff] hover:underline">View all workers </a>
          </div>
        </div>
...
// ==========================================
// MAIN DASHBOARD COMPONENT  AUTHENTICATION DISABLED
// ==========================================
export default function SovereignDashboard() {
  ...
  // System state polling  no token needed
  useEffect(() => {
    setMounted(true);
    ...
    const fetchSystemState = async () => {
      try {
        const healthRes = await fetchWithKey('/health');
        if (healthRes.ok) {
          const healthData = await healthRes.json();
          setKernelStatus('online');
          setWorkerCount(healthData.workers || 32);
          if (healthData.consciousness >= 7) setConsciousnessLevel('SENTIENT');
          else if (healthData.consciousness >= 3) setConsciousnessLevel('AWAKENING');
          else setConsciousnessLevel('FORMING');

          const eventRes = await fetchWithKey('/events/stats');
          if (eventRes.ok) {
  ...
  const handleTransmit = async () => {
    if (!prompt.trim() || isStreaming) return; // removed token check
    ...
    try {
      const response = await fetchWithKey('/kernel/stream', {
        method: 'POST',
        ...
        for (const part of parts) {
          ...
          try {
            ...
            if (data.type === 'thinking') {
              ...
              setMessages(prev => [...prev, { id: Date.now().toString(), role: 'system', content: ` ${data.content}`, timestamp: new Date().toLocaleTimeString() }]);
      ...
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'system', content: ` Connection Failed`, timestamp: new Date().toLocaleTimeString() }]);
    } finally {
  ...
  const heatmapData = Object.keys(eventStats.event_counts || {}).length > 0
    ...
    : { 'hive.memory.store': 120, 'orchestrator.intent': 85, 'worker.task': 60, 'cortex.recall': 20, 'kernel.stream': 45 };
  ...
  return (
    <div className="h-screen w-screen bg-[#030406] text-[#f5f5f7] font-sans overflow-hidden flex flex-col selection:bg-[#00E5FF]/30 selection:text-white relative p-4 gap-4 z-0">
      ...
      <main className="relative z-10 flex-1 flex gap-4 min-h-0">
        ...
        {/* RIGHT SIDEBAR: GLOSSARY + STATS */}
        <motion.div {...fadeUp} transition={{ ...springSoft, delay: 0.3 }} className="w-[320px] flex flex-col gap-4 shrink-0 min-h-0">
          <div className="shrink-0 bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-4 flex flex-col shadow-lg">
            <div className="flex justify-between items-center mb-2 border-b border-white/5 pb-2">
              <div className="flex items-center gap-2">
                <Network size={14} className="text-[#00E5FF]" />
                <span className="text-[10px] font-mono font-bold tracking-[0.2em] text-[#00E5FF] uppercase">EVENT BUS</span>
              </div>
              <span className="text-[8px] font-mono px-2 py-0.5 rounded bg-[#00E5FF]/10 text-[#00E5FF] border border-[#00E5FF]/20 flex items-center gap-1">
                <div className="w-1.5 h-1.5 bg-[#00E5FF] rounded-full animate-pulse" /> LIVE
              </span>
            </div>
            <div className="flex items-center justify-between mb-4">
              <div className="flex flex-col flex-1 border-r border-white/5 pl-2">
                <span className="text-[8px] text-zinc-500 font-mono tracking-widest">TOTAL</span>
                <span className="text-[14px] font-bold text-[#00E5FF]">{eventStats.total_events}</span>
              </div>
              <div className="flex-1 px-4 h-8 flex items-end">
                <svg width="100%" height="100%" preserveAspectRatio="none" className="overflow-visible">
                  <polyline
                    points={rateHistory.map((val, i) => `${(i / (rateHistory.length - 1)) * 100},${32 - (val * 3)}`).join(' ')}
                    fill="none"
                    stroke="#9B72CB"
                    strokeWidth="1.5"
                  />
                  <path
                    d={`M0,32 L${rateHistory.map((val, i) => `${(i / (rateHistory.length - 1)) * 100},${32 - (val * 3)}`).join(' L')} L100,32 Z`}
              ...
              <div className="flex flex-col flex-1 border-l border-white/5 pl-4">
                <span className="text-[8px] text-zinc-500 font-mono tracking-widest">RATE</span>
                <span className="text-[14px] font-bold text-[#9B72CB]">{eventRate}/s</span>
              </div>
            </div>
            <div className="flex flex-col gap-2 bg-[#030406]/50 p-3 rounded-xl border border-white/5">
              <span className="text-[8px] font-mono text-[#64748B] uppercase tracking-widest">Event Signatures</span>
              <div className="flex flex-wrap gap-1">
                {Object.entries(heatmapData).map(([key, count], i) => {
                  const intensity = Math.max(0.2, (count as number) / maxHeatmapCount);
                  return (
                    <div
                      key={key}
                      title={`${key}: ${count}`}
                      className="h-4 flex-1 min-w-[20px] rounded-[2px] transition-all hover:scale-110 cursor-help"
                      style={{ backgroundColor: `rgba(0, 229, 255, ${intensity})` }}
                    />
                  );
                })}
              </div>
